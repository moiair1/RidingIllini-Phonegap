	...
	$.ajax({
		...
		complete: function (data) {
			...
			if(flag){
				...
				var stopName="<li class='departures' id='first'>"+text+"<img class='favorites1' id='"+id+";"+text+"'src='img/favorite-yellow.png' height='50px' width='50px'/>"+"</li>";
			}
			else{
				var stopName="<li class='departures' id='first'>"+text+"<img class='favorites2' id='"+id+";"+text+"' src='img/favorite-white.png' height='50px' width='50px'/>"+"</li>";
			}
...
function displayDepartures(){
	$('#departure_wrap').show('fast','linear');
	$('#loader').html('');
...
function getNearest(position){
	...
	count=8-JSON.parse(localStorage.getItem("favorites")).length;
	if(count<3) count=3;
