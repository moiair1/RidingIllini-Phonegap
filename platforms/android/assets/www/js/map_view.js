var my_longitude,my_latitude,my_data,map,departureString,boxList=[],obj;
$(document).ready(function() {
	$('#loader').html("<video width='100%' autoplay loop><source src='img/loading-bar.mp4' type='video/mp4'/></video>");
	$('#loader').show();
	initialize();
	//$('#loader').hide();
...
function initialize() {
	navigator.geolocation.getCurrentPosition(draw, onError, {timeout: 10000, enableHighAccuracy: true});
}
...
function addStopMarkers(){
	for (var i = 0; i < my_data["stops"].length; i++) {
		...
		for(var j=0; j<my_data["stops"][i]["stop_points"].length; j++){
			...
			var string_creator="<div style='min-height:20vh;min-width:30vh; font-size:1em;'><em><u><p>"
			...
			google.maps.event.addListener(marker, "click", function() {
				this.infoWindow.setContent("<video autoplay loop> <source src='img/loading-bar.mp4' type='video/mp4'/></video>");
				obj=this;
				setTimeout('addToInfoWindow(obj)',2000);	
				//$('#loader').hide();
			});
		}
	}
}
function addToInfoWindow(obj){
	console.log(obj);
	getStopDepartures(obj.title);
	obj.infoWindow.setContent(departureString);
}
function getStopDepartures(input) {
	...
	$.ajax({
		...
		async:false,
		complete: function (data) {
			bindDepartureString(data,stop_name,stop_id);
		},
		error : function(jqXHR, textStatus, errorThrown) {
			alert('Network connection not found. Please restart the app.');
			console.log(jqXHR);
...
function bindDepartureString(data,stop_name,stop_id){
